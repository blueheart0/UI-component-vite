...
class DateScheduler implements DateSchedulerClass {
  private _tree = new IntervalTree<string>();
  private _map = new Map();

  constructor(arr: DateSchedulerEvent[] = []) {
    for (let item of arr) {
      this._tree.insert(
      ...
      let _search: DateSchedulerEventSearchResultItem[] = this._tree.search(
        ...
        (
          ...
          if (this._map.has(value)) {
            const friends: SearchOutput<DateSchedulerEvent> = this._tree.search(
              interval.output(),
            ...
            const filteredFriends: DateSchedulerEvent[] = friends.filter(
              item =>
